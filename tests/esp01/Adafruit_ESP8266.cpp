...
Adafruit_ESP8266::Adafruit_ESP8266(Stream *s, Stream *d, int8_t r) :
  stream(s), debug(d), reset_pin(r), host(NULL), writing(true) {
  setTimeouts();
};
...
boolean Adafruit_ESP8266::connectToAP(Fstr *ssid, Fstr *pass) {
  ...
  //    return false;
  //  }
  print(F("AT+CWJAP_CUR=\"")); // Join access point
  print(ssid);
  print(F("\",\""));
  print(pass);
  println('\"');
...
bool Adafruit_ESP8266::closeTCP(void) {
...
boolean Adafruit_ESP8266::requestURL(Fstr * url) {
  print(F("AT+CIPSEND="));
  println(27 + strlen_P((Pchr *)url) + strlen_P((Pchr *)host));
  if (find(F("> "))) { // Wait for prompt
    ...
    print(F("\r\n\r\n")); // 4
    //println();
    return (find(F("200"))); // Gets 'SEND OK' line
  }
  return false;
}


boolean Adafruit_ESP8266::sendPost(char* url, char* data ) {
  String dataLen = String(strlen(data));

  print(F("AT+CIPSEND="));
  println(94 + strlen_P((Pchr *)url) + strlen_P((Pchr *)host) + strlen(data) + dataLen.length());
  if (find(F("> "))) { // Wait for prompt
    print(F("POST ")); // 4
    print(url);
    print(F(" HTTP/1.1\r\nHost: ")); // 17
    print(host);
    print(F("\r\nContent-Type: application/json;charset=UTF-8")); //47
    print("\r\nContent-Length: "); //18+
    ...
    print(F("\r\n\r\n"));
    //println();
    return (find(F("200"), true)); // Gets 'SEND OK' line
  }
  return false;
}
